...
```bash
[afu@client-211 root]$ sudo mount -t ceph 192.168.13.231:6789:/ /mnt/cephfs -o name=admin,secret=AQBQOeVbsoiwJhAAPOJ6BC8KWAymEhugQjRRsw==
mount: 192.168.13.231:6789:/: can't read superblock
[afu@client-211 root]$
[afu@client-211 root]$
[afu@client-211 root]$ sudo dmesg | tail
...
[498834.545298] libceph: mon0 192.168.13.231:6789 session established
...
```bash
[ceph-admin@ceph-mon-1 prod-cluster]$ sudo ceph health detail
...
```bash
[ceph-admin@ceph-mon-1 prod-cluster]$ sudo ceph -s
  cluster:
  ...
  services:
    ...
    mds: gitlab_fs-1/1/1 up  {0=ceph-mon-3=up:active}, 2 up:standby
...
```bash
[ceph-admin@ceph-mon-1 prod-cluster]$ sudo ceph pg stat
